...
| 入口 | 文件 | 说明 |
|------|------|------|
| Popup | `extension/popup.tsx` | 路由 + 顶栏 + ErrorBoundary（196 行），页面组件在 `pages/` |
...
`extension/pages/` 存放 Popup 的页面组件（`HomePage.tsx`、`DetailPage.tsx`、`FavoritesPage.tsx`），从 `popup.tsx` 拆分而来。`extension/components/`（含 `ErrorState.tsx`、`GitStarIcon.tsx`、`SkeletonRepoCard.tsx` 等）和 `extension/hooks/` 存放 UI 组件和 hooks。类型定义在 `extension/lib/types.ts`，共享常量和 TTL 配置在 `extension/lib/constants.ts`（`CACHE_TTL` 集中管理缓存时间），缓存层在 `extension/lib/cache.ts`，SWR hook 在 `extension/hooks/useStaleCache.ts`，错误消息映射在 `extension/lib/errors.ts`，i18n 在 `extension/lib/i18n.tsx`（翻译字典在 `extension/locales/`），Worker 在 `extension/workers/markdown-worker.ts`。
...
Popup (popup.tsx + pages/*)   Content Script (contents/github-sidebar.tsx)
...
- **推荐算法**：调用 `getRepoInfo()`（不含 README）获取当前仓库 topics/language → 按 topic 相似度搜索高 Star 项目。
...
- **生命周期管理**：`SidebarLifecycle` IIFE 单例封装 `mount()` / `cleanup()` / `enabled`，集中管理 reactRoot/mountTimeout/mountObserver/lastUrl/sidebarEnabled 五个可变状态，防止多次注入不一致。
- **手动挂载 + SPA 适配**：Content Script 不依赖 Plasmo 自动注入，而是 `SidebarLifecycle.mount()` 手动挂载。必须提供 `export default`（返回 null）避免 Plasmo 自动渲染报 Error #130（见已知陷阱）。通过 `MutationObserver` 观察 `<title>` 变化检测 GitHub SPA 导航（替代 setInterval 轮询），自动 `cleanup()` → 重新挂载。优先查 `#repo-details-container` → `.Layout-sidebar` → `aside`，未找到时用 `MutationObserver` 等待 DOM 出现，10 秒超时后回退为 `position: fixed` 浮动面板。
- **推荐缓存**：推荐结果按 `owner/repo` 做 60 秒内存缓存（`recsCache` Map），同一仓库内快速切换不会重复请求 API。
- **开关控制**：通过选项页的 `gitstar-sidebar-enabled` 开关（`chrome.storage.local`）控制是否注入。`SidebarLifecycle.enabled` 为 false 时跳过挂载。监听 `chrome.storage.onChanged` 实现选项页切换即时生效，无需刷新页面。
- **无障碍**：折叠按钮支持 `role="button"` + `tabIndex={0}` + Enter/Space 键盘操作。
