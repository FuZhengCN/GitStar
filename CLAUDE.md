...
| Options | `extension/options.tsx` | Token + AI 配置页（`OptionsIndex` 外层 Provider + `OptionsForm` 内层） |
...
chrome.storage.local → githubToken + gitstar-favorites（本地收藏）+ gitstar-cache:*（API 数据缓存）+ gitstar-lang（语言偏好，设备级）+ gitstar-ai-config（AI 配置）+ gitstar-ai:summary:v3:*（AI 概述缓存，独立前缀）
