...
- **Chrome popup 高度限制**：macOS 上 Chrome 扩展 popup 内容视口硬限 600px（`window.innerHeight`），CSS `100vh`、`height: 100%` 链、`window.resizeTo`、`chrome.windows.update` 均无法突破。不要试图控制 popup 窗口高度，应以 600px 为设计基准。
...
| 新标签页模式 | 06-09 | `specs/2026-06-09-gitstar-tab-mode-design.md` | `plans/2026-06-09-gitstar-tab-mode-plan.md` |
...
Plasmo v0.90.5 入口文件（根级和 `tabs/` 目录）：

| 入口 | 文件 | 说明 |
|------|------|------|
| Popup | `extension/popup.tsx` | 工具栏弹窗（400px），wouter hash 路由壳 |
| Tab | `extension/tabs/tab.tsx` | 新标签页全宽入口，wouter hash 路由壳 |
| Options | `extension/options.tsx` | Token + AI 配置 + 打开方式（`OptionsIndex` 外层 Provider + `OptionsForm` 内层） |
| Background | `extension/background.ts` | Service Worker，`chrome.action.setPopup` 动态分发 popup/tab 模式 |

**页面组件（popup 和 tab 共用）：** `extension/components/HomePage.tsx`、`DetailPage.tsx`、`FavoritesPage.tsx` — 从 popup.tsx 提取，通过 `layout` prop（`'popup' | 'tab'`）控制布局差异。popup 模式 fixed 底栏 + 单列卡片，tab 模式流式布局 + 响应式双列。

**共享基础设施：** `extension/components/`（含 `ErrorBoundary.tsx`、`ErrorState.tsx`、`GitStarIcon.tsx` 等）和 `extension/hooks/`。类型定义在 `extension/lib/types.ts`，共享常量在 `extension/lib/constants.ts`（含 `POPUP_WIDTH`、`OPEN_MODE_STORAGE_KEY`），缓存层在 `extension/lib/cache.ts`，SWR hook 在 `extension/hooks/useStaleCache.ts`，AI 摘要模块在 `extension/lib/ai-summary.ts`（含 `parseAiSections`、`escapeHtml`），i18n 在 `extension/lib/i18n.tsx`（含 `errorMessageText`），Worker 在 `extension/workers/markdown-worker.ts`。
...
extension/lib/ai-summary.ts   ← 智能截断 + OpenAI 兼容 API 调用 + 独立缓存（v3 前缀，≤50 条）
...
chrome.storage.local → githubToken + gitstar-favorites（本地收藏）+ gitstar-open-mode（打开方式：popup/tab）+ gitstar-cache:*（API 数据缓存）+ gitstar-lang（语言偏好，设备级）+ gitstar-ai-config（AI 配置）+ gitstar-ai:summary:v3:*（AI 概述缓存，独立前缀）+ gitstar-mode（发现模式偏好）
sessionStorage        → 搜索参数（popup 关闭即清除；tab 标签页关闭才清除）
```
...
**permissions 说明：** 当前 `["storage", "tabs"]`。`tabs` 权限用于 `background.ts` 中 `chrome.tabs.query`（标签页去重，URL 匹配已打开的 tab 页面）和 `chrome.windows.update`（聚焦后台窗口）。审核接受度高。
...
README content → truncateReadme()（去图/去代码/截断 8KB）
  ...
  → 查缓存 gitstar-ai:summary:v3:<owner>/<repo>（独立前缀，最多 50 条，无 TTL）
...
**缓存独立前缀：** AI 摘要缓存用 `gitstar-ai:summary:v3:` 前缀（注意版本号），与通用 `gitstar-cache:` 分离，避免被 LRU 淘汰（通用池 30 条）。自身最多 50 条，按时间戳淘汰旧条目。
...
Popup 宽度固定 400px（`POPUP_WIDTH`），外层 `min-h-[720px]` + `flex flex-col`，内容区 `flex-1` 撑满可用高度。Chrome 在 macOS 上将 popup 内容视口硬限制为 600px（窗口总高可达 `screen.availHeight`，但内高固定 600px），CSS 高度链和视口单位均不可靠，应以内容自然高度为基准设计。
...
**共享结构：** 蓝底顶栏 `position: fixed; top: 0; z-30`（`w-[400px]`），内容区有 `p-4 pt-[60px]` 避开 Header 阴影（`shadow-[0_2px_8px_rgba(59,130,246,0.25)]` 向下方延展约 10px）。外层容器 `bg-slate-50`，内容卡片 `bg-white` + 阴影浮起（无硬边框）。`HomePage` 和 `DetailPage` 各自渲染内容，不重复顶栏和 padding。
...
- **Plasmo 文件约定**：入口是 `popup.tsx` / `options.tsx`（根级），不是 `popup/index.tsx`。
...
- **Popup 高度不可控**：Chrome 根据内容自然高度决定 popup 窗口大小（上限 600px 内高）。`h-screen`、`height: 100%` 级联、`100vh` 在 popup 内均不可靠。不要试图通过 CSS 或 JS 设置 popup 高度，应让内容自然撑开，用 `position: fixed` 将需要常显的元素（分页等）固定在视口内。
...
- **i18n — I18nProvider 必须在两个入口分别包裹**：Popup/Options 是两个独立 React 树（不同 `createRoot`），每个都需要自己的 `<I18nProvider>`。Provider 通过 `chrome.storage.onChanged` 监听 `gitstar-lang` 键实现跨上下文语言同步。
...
- **AI 概述 section 解析在 ai-summary.ts 中**：`parseAiSections()`（含 `escapeHtml()`、`ParsedSection` 接口）位于 `extension/lib/ai-summary.ts`，同时处理全角冒号（`：`）和 ASCII 冒号（`: `），支持多行内容累积（LLM 输出的列表项）。popup 和 tab 页共用。
- **新标签页模式入口**：`tabs/tab.tsx`（非根级 `tab.tsx`）。Plasmo 将 `tabs/` 目录下的文件构建为独立页面，输出路径 `tabs/tab.html`。不要将标签页入口放在根级，否则不会生成 HTML。`background.ts` 中 `TAB_PATH = 'tabs/tab.html'`，与构建产物路径一致。
- **`PageLayout` 类型和 `layout` prop**：共享页面组件（HomePage/DetailPage/FavoritesPage）通过 `layout: 'popup' | 'tab'` prop 区分布局。popup → fixed 底栏 + 单列卡片；tab → 流式布局 + 响应式双列。新增 UI 差异时走同一 prop，不要引入新的 mode 检测机制。
- **`chrome.action.setPopup({ popup: '' })`**：传空串（非 null）禁用 popup，使 `chrome.action.onClicked` 生效。`setPopup` 是持久化的（跨 SW 生命周期），不会因 SW 终止而回退到 manifest default_popup。
- **URL 通配符匹配**：`background.ts` 中 `chrome.tabs.query({ url: url + '*' })` 需要 `*` 后缀，因为标签页实际 URL 带 hash（如 `#/project/vue/core`），不加通配符匹配不到。
